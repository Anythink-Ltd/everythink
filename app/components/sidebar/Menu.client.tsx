...
function CurrentDateTime() {
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 text-sm text-zinc-600 dark:text-zinc-400 border-b border-zinc-100 dark:border-zinc-800/50">
    ...
    <>
      <motion.div
        ...
        className={classNames(
          ...
          'bg-white/20 dark:bg-zinc-950/20 backdrop-blur-xl border-r border-bolt-elements-borderColor',
          'shadow-sm text-sm',
          isSettingsOpen ? 'z-40' : 'z-sidebar',
        )}
      >
        <div className="h-12 flex items-center justify-between px-4 border-b border-zinc-100/20 dark:border-zinc-800/20 bg-zinc-50/10 dark:bg-zinc-900/10 rounded-tr-2xl">
          <div className="text-zinc-900 dark:text-white font-medium"></div>
          <div className="flex items-center gap-3">
            <HelpButton onClick={() => window.open('https://anythink.ai/docs', '_blank')} />
            <span className="font-medium text-sm text-zinc-900 dark:text-white truncate">
              {profile?.username || 'Guest User'}
            </span>
            <div className="flex items-center justify-center w-[32px] h-[32px] overflow-hidden bg-white dark:bg-zinc-800 text-zinc-600 dark:text-zinc-500 rounded-full shrink-0">
              {profile?.avatar ? (
                <img
                  src={profile.avatar}
        ...
        <div className="flex-1 flex flex-col h-full w-full overflow-hidden">
          <div className="p-4 space-y-3">
            ...
            <div className="relative w-full">
              <div className="absolute left-3 top-1/2 -translate-y-1/2">
                <span className="i-ph:magnifying-glass h-4 w-4 text-zinc-400 dark:text-zinc-500" />
              </div>
              <input
                className="w-full bg-zinc-50 dark:bg-zinc-900 relative pl-9 pr-3 py-2 rounded-lg focus:outline-none focus:ring-1 focus:ring-orange-500/50 text-sm text-zinc-900 dark:text-zinc-100 placeholder-zinc-500 dark:placeholder-zinc-500 border border-zinc-200 dark:border-zinc-800"
                type="search"
          ...
          <div className="flex items-center justify-between text-sm px-4 py-2">
            <div className="font-medium text-zinc-600 dark:text-zinc-400">Your Chats</div>
            {selectionMode && (
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-3 pb-3">
            {filteredList.length === 0 && (
              <div className="px-4 text-zinc-500 dark:text-zinc-400 text-sm">
            ...
            <DialogRoot open={dialogContent !== null}>
              {binDates(filteredList).map(({ category, items }) => (
                <div key={category} className="mt-2 first:mt-0 space-y-1">
                  <div className="text-xs font-medium text-zinc-500 dark:text-zinc-400 sticky top-0 z-1 bg-white dark:bg-zinc-950 px-4 py-1">
                    {category}
                  </div>
              ...
              <Dialog onBackdrop={closeDialog} onClose={closeDialog}>
                {dialogContent?.type === 'delete' && (
                  <>
                    <div className="p-6 bg-white dark:bg-zinc-950">
                      <DialogTitle className="text-zinc-900 dark:text-white">Delete Chat?</DialogTitle>
                    ...
                    <div className="flex justify-end gap-3 px-6 py-4 bg-zinc-50 dark:bg-zinc-900 border-t border-zinc-100 dark:border-zinc-800">
                ...
                {dialogContent?.type === 'bulkDelete' && (
                  <>
                    <div className="p-6 bg-white dark:bg-zinc-950">
                      <DialogTitle className="text-zinc-900 dark:text-white">Delete Selected Chats?</DialogTitle>
                      <DialogDescription className="mt-2 text-zinc-600 dark:text-zinc-400">
                        <p>
                          You are about to delete {dialogContent.items.length}{' '}
                          {dialogContent.items.length === 1 ? 'chat' : 'chats'}:
                        </p>
                        <div className="mt-2 max-h-32 overflow-auto border border-zinc-100 dark:border-zinc-800 rounded-md bg-zinc-50 dark:bg-zinc-900 p-2">
                          <ul className="list-disc pl-5 space-y-1">
                            {dialogContent.items.map((item) => (
                              <li key={item.id} className="text-sm">
                                <span className="font-medium text-zinc-900 dark:text-white">{item.description}</span>
                              </li>
                            ))}
                          </ul>
                        </div>
